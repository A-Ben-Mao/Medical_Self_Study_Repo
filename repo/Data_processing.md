...
做实验数据处理离不开几个常用软件，这里把下载和使用笔记汇总在一起，免得到处翻。
...
免疫荧光染完后通常得统计细胞数量。手动数效率低、重复性差，ImageJ 可以批量自动完成，而且结果可复现。
...
双标染色后需要判断不同标记之间是否存在共定位关系，或者统计双阳性细胞的比例。以下流程基于 ImageJ 实现。
...
WB 曝光后拿到条带图，需要将条带的灰度值转化为可比较的定量数据。ImageJ 可以快速完成从条带到数值的转换。
